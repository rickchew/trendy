...
export class OrderPage {
  ...
  initializePaymentMethods() {
    // this.loading.show();
    this.http.get(this.config.url + 'getPaymentMethods').map(res => res.json()).subscribe(data => {
      //  this.loading.hide();
      if (data.success == 1) {
        for (let a of data.data) {
          if (a.name == "Braintree" && a.active == '0') { this.paymentBraintree = false; }
          if (a.name == "Stripe" && a.active == '0') { this.paymentStripe = false; }
          if (a.name == "Cash On Delivery" && a.active == '0') { this.paymentCod = false; }
          if (a.name == "Paypal" && a.active == '0') { this.paymentPaypal = false; }

          if (a.name == "Paypal" && a.active == '1') {
          ...
          if (a.name == "Stripe" && a.active == '1') { this.stripe.setPublishableKey(a.public_key); }
        }
        // var key = data.data[0].publishable_key;
